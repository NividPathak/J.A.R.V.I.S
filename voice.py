...
def main() -> int:
    ...
    # Load the agent model before the first question. MLX (Whisper, tuned
    # router) and Ollama compete for memory, so an unwarmed model costs ~16s on
    # the first turn — spend it here, while nothing is waiting on it.
    warm = getattr(getattr(orchestrator, "agents", {}).get("weather", None), "_llm", None)
    if hasattr(warm, "warm"):
        console.print("[dim]Warming the agent model…[/]")
        warm.warm()
    ...
    while True:
        try:
            ...
            turn(orchestrator, heard, speak=not args.no_speak)
